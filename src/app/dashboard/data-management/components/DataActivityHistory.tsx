...
const DataActivityHistory: React.FC<DataActivityHistoryProps> = ({ dataActivities, isMobile }) => {
  ...
  // Show mobile-optimized view if isMobile is true
  if (isMobile) {
    return (
      <div className="bg-white rounded-[20px] p-4 shadow-md border border-gray-100 mb-4">
        <h3 className="text-[16px] font-bold text-[#1E1E1E] mb-3">{t('Data Activity History')}</h3>
        
        {/* Mobile cards view */}
        <div className="space-y-3">
          {dataActivities.map(activity => (
            <div key={activity.id} className="bg-[#F9F9F9] rounded-[15px] p-3">
