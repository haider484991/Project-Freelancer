  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      {statsData.map((stat, index) => (
        <div 
          key={index}
          className="bg-gradient-to-b from-[#13A753] to-[#1E2120] rounded-xl p-4"
        >
          <div className="flex flex-col gap-2">
            <div className="flex items-start justify-between">
              <h3 className="text-[20px] font-extrabold text-white capitalize">
                {stat.value}
              </h3>
              <div className="w-[32px] h-[32px] rounded-full bg-white/25 flex items-center justify-center">
                {stat.icon}
              </div>
            </div>
